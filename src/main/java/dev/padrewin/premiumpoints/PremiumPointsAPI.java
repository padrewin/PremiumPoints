...
/**
 * The API for the PremiumPoints plugin.
...
public class PremiumPointsAPI {
    ...
    public boolean give(@NotNull UUID playerId, int amount) {
        ...
        PremiumPointsChangeEvent event = new PremiumPointsChangeEvent(playerId, amount);
    ...
    public boolean set(@NotNull UUID playerId, int amount) {
        ...
        PremiumPointsChangeEvent event = new PremiumPointsChangeEvent(playerId, amount - points);
        Bukkit.getPluginManager().callEvent(event);
        if (event.isCancelled())
            return false;
    ...
    public boolean reset(@NotNull UUID playerId) {
        ...
        PremiumPointsResetEvent event = new PremiumPointsResetEvent(playerId);
        Bukkit.getPluginManager().callEvent(event);
        if (event.isCancelled())
            return false;
