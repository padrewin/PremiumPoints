...
import dev.padrewin.premiumpoints.database.migrations._1_Create_Tables;
import dev.padrewin.premiumpoints.database.migrations._2_Add_Table_Username_Cache;
import dev.padrewin.premiumpoints.listeners.PointsMessageListener;
import dev.padrewin.premiumpoints.models.PendingTransaction;
...
public class DataManager extends AbstractDataManager implements Listener {
    ...
    public void reload() {
        ...
        this.pointsCache = CacheBuilder.newBuilder()
                ...
                .refreshAfterWrite(dev.padrewin.premiumpoints.setting.SettingKey.CACHE_DURATION.get(), TimeUnit.SECONDS)
    ...
    private int getPoints(UUID playerId) {
        ...
        if (generate.get()) {
            int startingBalance = dev.padrewin.premiumpoints.setting.SettingKey.STARTING_BALANCE.get();
        ...
        this.databaseConnector.connect(connection -> {
            ...
            try (PreparedStatement statement = connection.prepareStatement(query)) {
                for (Map.Entry<UUID, Integer> entry : transactions.entrySet()) {
                    ...
                    if (dev.padrewin.premiumpoints.setting.SettingKey.BUNGEECORD_SEND_UPDATES.get() && this.coldPlugin.isEnabled()) {
