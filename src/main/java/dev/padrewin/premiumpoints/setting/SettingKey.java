...
public class SettingKey {
    ...
    public static final ColdSetting<String> VAULT_PRIORITY = create("vault-priority", STRING, "Low", "The priority level to use for the Vault hook", "Higher priorities will allow PremiumPoints to load before other economy plugins", "Valid values: [Lowest, Low, Normal, High, Highest]");
    ...
    private static ColdSetting<CommentedConfigurationSection> create(String key, String... comments) {
        ColdSetting<CommentedConfigurationSection> setting = ColdSetting.backedSection(PremiumPoints.getInstance(), key, comments);
        KEYS.add(setting);
        return setting;
    }
