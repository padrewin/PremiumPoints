...
import dev.padrewin.playerpoints.database.migrations._1_Create_Tables;
import dev.padrewin.playerpoints.database.migrations._2_Add_Table_Username_Cache;
import dev.padrewin.playerpoints.listeners.PointsMessageListener;
import dev.padrewin.playerpoints.models.PendingTransaction;
import dev.padrewin.playerpoints.models.SortedPlayer;
...
public class DataManager extends AbstractDataManager implements Listener {
    ...
    public void reload() {
        ...
        this.pointsCache = CacheBuilder.newBuilder()
                ...
                .refreshAfterWrite(dev.padrewin.playerpoints.setting.SettingKey.CACHE_DURATION.get(), TimeUnit.SECONDS)
    ...
    private int getPoints(UUID playerId) {
        ...
        if (generate.get()) {
            int startingBalance = dev.padrewin.playerpoints.setting.SettingKey.STARTING_BALANCE.get();
        ...
        this.databaseConnector.connect(connection -> {
            ...
            try (PreparedStatement statement = connection.prepareStatement(query)) {
                for (Map.Entry<UUID, Integer> entry : transactions.entrySet()) {
                    ...
                    if (dev.padrewin.playerpoints.setting.SettingKey.BUNGEECORD_SEND_UPDATES.get() && this.coldPlugin.isEnabled()) {
                        ...
                        if (attachedPlayer != null)
                            attachedPlayer.sendPluginMessage(this.coldPlugin, PointsMessageListener.CHANNEL, output.toByteArray());
                    }
                }
    ...
    private String getPointsTableName() {
        if (dev.padrewin.playerpoints.setting.SettingKey.LEGACY_DATABASE_MODE.get()) {
            return dev.padrewin.playerpoints.setting.SettingKey.LEGACY_DATABASE_NAME.get();
        } else {
